...
class _VitalsPageState extends State<VitalsPage> {
  ...
  double _distanceFromHome = 0;

  // Safe zone — set to patient's home coordinates
  static const double _safeZoneLat    = 19.0760;
  static const double _safeZoneLng    = 72.8777;
  static const double _safeZoneRadius = 500; // metres
  ...
  // ── Firestore listener 1: users/{patientId} (live status) ────
  // Unchanged path — same as original code
  void _listenToFirestore() {
    _firestoreSub = FirebaseFirestore.instance
      ...
      // Bed state from ESP32 (new)
      final bedState = sensors['bed_status']?.toString() ?? 'UNKNOWN';

      // Location check trigger from ESP32 (new)
      final checkLocation = sensors['checkLocationNow'] == true;
      ...
      // Location check triggered by ESP32 when patient leaves bed
      if (checkLocation && !_locationChecked) {
        _locationChecked = true;
        _runLocationCheck();
      }
      if (!checkLocation) _locationChecked = false;
    });
  }
  ...
  // ── Location check (runs when ESP32 sets checkLocationNow) ───
  Future<void> _runLocationCheck() async {
    try {
      ...
      final dist = Geolocator.distanceBetween(
        _safeZoneLat, _safeZoneLng,
        pos.latitude, pos.longitude,
      );

      final inZone = dist <= _safeZoneRadius;
      ...
      await FirebaseFirestore.instance
          ...
          .doc(widget.patientId)
          .update({
        'sensors.currentLat':      pos.latitude,
        'sensors.currentLng':      pos.longitude,
        'sensors.isInSafeZone':    inZone,
        'sensors.distanceFromHome': dist,
        'sensors.checkLocationNow': false, // clear the flag
      });

      setState(() {
        _isInSafeZone     = inZone;
        _distanceFromHome = dist;
      });

      if (!inZone && mounted) _showOutOfZoneAlert(dist);
    } catch (e) {
      debugPrint('[Location] Error: $e');
    }
  }

  void _showOutOfZoneAlert(double dist) {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'Patient is ${dist.toStringAsFixed(0)}m from home.',
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              // Alert banners (shown above blob chain when active)
              if (_isUnresponsive) _alertBanner(
                  '🔴 EMERGENCY — Patient Unresponsive!', Colors.red),
              if (_isHypersomnia && !_isUnresponsive) _alertBanner(
                  '🟡 HYPERSOMNIA — In bed over 13 hours', Colors.orange),
              if (!_isInSafeZone) _alertBanner(
                  '⚠️ Patient outside safe zone — '
                      '${_distanceFromHome.toStringAsFixed(0)}m from home',
                  Colors.deepOrange),

              const SizedBox(height: 12),

              // Original blob chain — completely unchanged structure
              _buildBlobChain(),

              const SizedBox(height: 32),

              // Bar graphs below blob chain
  ...
  // ── Blob chain (original — only _buildChart() updated inside) ─
      ...
      return SizedBox(
        ...
        child: Stack(
          ...
          children: [
            ...
            // STATUS — now shows real bed state
            Positioned(
            ...
            // VELOSTAT GRAPH — line chart now shows real micro-movement data
            Positioned(
  ...
  Widget _buildChart() {
    // Use dummy spots if no real data yet
    ...
    final maxY = _microMovementSpots.isNotEmpty
        ? (_microMovementSpots.map((s) => s.y).reduce((a, b) => a > b ? a : b)
        * 1.3)
        .clamp(10.0, 200.0)
        : 50.0;
  ...
  // ── Bar graph section (added below blob chain) ────────────────
